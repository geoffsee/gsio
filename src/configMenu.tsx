...
export const ConfigMenu = () => {
				...
				if (val.length > 0) {
					if (mode === "editAllow") {
						const next = {
							...
							shell: {
								...cfg.shell,
								extraAllowlist: [...cfg.shell.extraAllowlist, val],
							},
						};
						setCfg(next);
						...
						const next = {
							...
							audio: {
								...cfg.audio,
								whisper: { ...cfg.audio.whisper, command: val },
							},
						};
						setCfg(next);
						...
						const next = {
							...
							audio: {
								...cfg.audio,
								whisper: { ...cfg.audio.whisper, model: val },
							},
						};
						setCfg(next);
						...
						if (!cfg.tools.requireApproval.includes(val)) {
							const next = {
								...
								tools: {
									...cfg.tools,
									requireApproval: [...cfg.tools.requireApproval, val],
								},
							};
							setCfg(next);
						}
		...
		if (ch === "d" || ch === "D") {
			...
			if (it?.kind === "allow") {
				...
				const next = {
					...
					shell: {
						...cfg.shell,
						extraAllowlist: cfg.shell.extraAllowlist.filter(
							(_, i) => i !== idx
						),
					},
				};
			...
			if (it?.kind === "approval") {
				...
				const next = {
					...
					tools: {
						...cfg.tools,
						requireApproval: cfg.tools.requireApproval.filter(
							(_, i) => i !== idx
						),
					},
				};
	...
	return (
		<Box flexDirection="column">
			...
			<Text color="gray">
				↑/↓ select • Space toggle • ←/→ adjust • a add allow • t add approval •
				d delete • e edit behavior • s save • Esc exit
			</Text>
			<Newline />
			{items.map((it, i) => (
				<Text
					key={`${it.key}-${i}`}
					color={i === index ? "magenta" : undefined}
				>
					{i === index ? "› " : "  "}
					{renderItem(it)}
				</Text>
	...
	function buildItems(c: AppConfig) {
		...
		out.push({
			...
			toggle: () =>
				setCfg({
					...
					ai: {
						...c.ai,
						provider: c.ai.provider === "openai" ? "ollama" : "openai",
					},
				}),
			kind: "toggle",
		});
		...
		out.push({
			...
			action: () => {
				...
				setInput(
					c.ai.baseUrl ||
						(c.ai.provider === "ollama" ? "http://localhost:11434/v1" : "")
				);
			},
			kind: "action",
		});
		out.push({
			...
			toggle: () =>
				setCfg({
					...c,
					shell: { ...c.shell, allowDangerous: !c.shell.allowDangerous },
				}),
			kind: "toggle",
		});
		out.push({
			...
			toggle: () =>
				setCfg({ ...c, linger: { ...c.linger, enabled: !c.linger.enabled } }),
			kind: "toggle",
		});
		out.push({
			key: "linger.behavior",
		...
		out.push({
			...
			inc: () =>
				setCfg({
					...
					linger: {
						...c.linger,
						minIntervalSec: Math.min(600, c.linger.minIntervalSec + 5),
					},
				}),
			dec: () =>
				setCfg({
					...
					linger: {
						...c.linger,
						minIntervalSec: Math.max(5, c.linger.minIntervalSec - 5),
					},
				}),
			kind: "number",
		});
		out.push({
			...
			toggle: () =>
				setCfg({
					...c,
					audio: { ...c.audio, captureEnabled: !c.audio.captureEnabled },
				}),
			kind: "toggle",
		});
		out.push({
			...
			toggle: () =>
				setCfg({
					...
					audio: {
						...c.audio,
						sttProvider:
							c.audio.sttProvider === "openai" ? "whisper" : "openai",
					},
				}),
			kind: "toggle",
		});
		if (c.audio.sttProvider === "whisper") {
			out.push({
				...
				action: () => {
					setMode("editWhisperCmd");
					setInput(c.audio.whisper.command || "whisper-cpp");
				},
				kind: "action",
			});
			out.push({
				...
				label: `Whisper model: ${
					c.audio.whisper.model || "(set path to .bin)"
				}`,
				action: () => {
					setMode("editWhisperModel");
					setInput(c.audio.whisper.model || "");
				},
				kind: "action",
			});
		}
		out.push({
			...
			toggle: () =>
				setCfg({
					...c,
					panel: { ...c.panel, todoShowCompleted: !c.panel.todoShowCompleted },
				}),
			kind: "toggle",
		});
		out.push({
			...
			inc: () =>
				setCfg({
					...c,
					panel: { ...c.panel, maxItems: Math.min(20, c.panel.maxItems + 1) },
				}),
			dec: () =>
				setCfg({
					...c,
		...
		out.push({
			key: "allow.header",
			label: "Shell allowlist (press a to add, d to delete)",
			kind: "header",
		});
		for (let i = 0; i < c.shell.extraAllowlist.length; i++) {
			out.push({
				key: `allow.${i}`,
				label: `  - ${c.shell.extraAllowlist[i]}`,
				kind: "allow",
				idx: i,
			});
		}
		out.push({
			key: "approval.header",
			label: "Tools requiring approval (press t to add, d to delete)",
			kind: "header",
		});
		for (let i = 0; i < c.tools.requireApproval.length; i++) {
			out.push({
				key: `approval.${i}`,
				label: `  - ${c.tools.requireApproval[i]}`,
				kind: "approval",
				idx: i,
			});
		}
