...
export async function loadStore(): Promise<TodoStore> {
	...
	const items: Todo[] = (Array.isArray(data.items) ? data.items : []).map(
		(raw: any): Todo => {
			...
			return {
				...
				status,
				priority: [1, 2, 3, 4, 5].includes(Number(raw.priority))
					? (Number(raw.priority) as any)
					: 3,
...
export async function listTodos(includeCompleted = true): Promise<Todo[]> {
	...
	const items = includeCompleted
		? store.items
		: store.items.filter((t) => t.status !== "done");
...
export async function setStatus(
	...
	todo.status = status;
	todo.blockedReason =
		status === "blocked" ? blockedReason || "unspecified" : undefined;
...
export async function addNoteToTodo(
	id: number,
	note: string
): Promise<Todo | undefined> {
	const store = await loadStore();
...
export async function setFocus(id: number | null): Promise<number | null> {
	const store = await loadStore();
	if (id !== null && !store.items.find((t) => t.id === id))
