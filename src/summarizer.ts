...
Avoid duplicating content; integrate updates succinctly.`;
  ...
  const {baseUrl: resolvedBaseUrl, note: baseUrlNote} = normalizeBaseUrl(provider, rawBaseUrl);
  if (baseUrlNote) {
    console.warn(`[summarizeAudioContext] ${baseUrlNote} (provider=${provider}, originalBaseUrl=${rawBaseUrl})`);
  }
  const apiKey = (cfg.ai?.apiKey ?? '').trim() || process.env['OPENAI_API_KEY'] || '';
...
function normalizeBaseUrl(provider: string, raw: string): {baseUrl: string; note: string | null} {
